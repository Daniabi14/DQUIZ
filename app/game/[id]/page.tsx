...
export default function StudentGamePage() {
  ...
  useEffect(() => {
    if (currentQuestion && currentQuestion.id !== lastAnsweredQId) {
      // Check if already answered in local cache
      ...
      // Shuffle options for this participant if enabled in settings
  ...
  const handleSelectOption = async (optionId: string) => {
    ...
    // Submit answer to server-authoritative engine
    ...
    <div className="flex-1 flex flex-col items-center justify-between p-4 sm:p-6 max-w-xl mx-auto w-full min-h-[calc(100vh-4rem)]">
      ...
      <div className="w-full flex-1 flex flex-col items-center justify-center">
        <AnimatePresence mode="wait">
          {/* 1. LOBBY WAITING ROOM (Prompt item 8) */}
          ...
          {/* 2. LIVE QUESTION SCREEN (Prompt items 9, 10) */}
          {game && game.status === "QUESTION" && currentQuestion && (
            ...
            >
              {/* Question Header & Authoritative Timer */}
              ...
              {/* Submitted State Banner (Prompt item 10) */}
              {isSubmitted && (
                <motion.div
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
          ...
          {/* 3. QUIZ PAUSED BANNER (Prompt item 37) */}
          ...
          {/* 4. BETWEEN QUESTIONS NEUTRAL SCREEN (Prompt item 11) */}
          ...
          {/* 5. FINAL STUDENT COMPLETION SCREEN (Prompt item 12) */}
